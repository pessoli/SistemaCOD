...
public class DespesaService {
    ...
    public void atualizarValorDespesa(Long id, double valorPago) {
        // Verifica se o ID não é nulo
        if (id == null) {
            throw new IllegalArgumentException("O ID da despesa não pode ser nulo.");
        }

        // Tenta encontrar a despesa pelo ID
        despesaRepository.findById(id).ifPresentOrElse(
                despesa -> {
                    // Atualiza o valor da despesa
                    despesa.setValor(valorPago);
                    despesaRepository.save(despesa);  // Salva a despesa atualizada no banco
                },
                () -> {
                    // Caso a despesa não seja encontrada
                    throw new EntityNotFoundException("Despesa com ID " + id + " não encontrada.");
                }
        );
    }

    public Double somarDespesasPorTipo(Long idTipoDespesa) {
        return despesaRepository.somarDespesasPorTipo(idTipoDespesa);
    }
    ...
    public void atualizarPagamento(Long id) {
        ...
        despesaRepository.findById(id).ifPresentOrElse(
                despesa -> {
                    // Aqui você pode fazer qualquer tipo de atualização necessária
                    // Por exemplo, se precisar atualizar algum campo específico
                    // Exemplo: despesa.setValor(novoValor);

                    despesaRepository.save(despesa);  // Salva as alterações no banco
                },
                () -> {
                    // Caso a despesa não seja encontrada
                    throw new EntityNotFoundException("Despesa com ID " + id + " não encontrada.");
